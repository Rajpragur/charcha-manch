...
const convertCandidateToConstituency = (
  candidate: any,
  index: number,
): ConstituencyData => {
  const result = {
    ...
    fundsUtilization:
      candidate.vidhayak_info.metadata?.funds_utilisation || "N/A",
    education: {
...
const Home: React.FC<HomeProps> = () => {
  ...
  const allConstituencies = useMemo(() => {
    ...
    const converted = candidatesData.map((candidate, index) =>
      convertCandidateToConstituency(candidate, index),
    );
    return converted;
  }, [candidatesData]);
  ...
  const content = {
    ...
    workDiscussion: isEnglish
      ? "किसने किया है कितना काम आओ करे चर्चा"
      : "किसने किया है कितना काम आओ करे चर्चा",
    searchPlaceholder: isEnglish
      ? "Search by constituency, candidate, or party..."
      : "निर्वाचन क्षेत्र, उम्मीदवार, या पार्टी से खोजें...",
    nagrikYogdan: isEnglish ? "Citizen Contribution" : "नागरिक योगदान",
    achievements: isEnglish ? "Our Achievements" : "हमारी उपलब्धियां",
    charchitVidhanSabha: isEnglish
  ...
  const quickStats = [
    {
      label: isEnglish ? "Total Constituencies" : "कुल निर्वाचन क्षेत्र",
      value: "243",
      icon: <MapPin className="h-5 w-5" />,
      color: "text-sky-600",
    },
    {
      label: isEnglish ? "Active Users" : "सक्रिय उपयोगकर्ता",
      value: "2,780",
      icon: <Users className="h-5 w-5" />,
      color: "text-emerald-600",
    },
    {
      label: isEnglish ? "Discussions" : "चर्चाएं",
      value: "1,245",
      icon: <MessageSquare className="h-5 w-5" />,
      color: "text-amber-600",
    },
    {
      label: isEnglish ? "Votes Cast" : "डाले गए वोट",
      value: "15,890",
  ...
  const recentActivities = [
    {
      type: "discussion",
      text: isEnglish
        ? "New discussion started in Patna Central"
        : "पटना सेंट्रल में नई चर्चा शुरू",
      time: "2 hours ago",
    ...
    },
    {
      type: "vote",
      text: isEnglish
        ? "Voting completed in Gaya constituency"
        : "गया निर्वाचन क्षेत्र में मतदान पूरा",
      time: "5 hours ago",
    ...
    },
    {
      type: "news",
      text: isEnglish
        ? "Development project announced in Banka"
        : "बांका में विकास परियोजना की घोषणा",
      time: "1 day ago",
  ...
  const filteredAndSortedConstituencies = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      filtered = allConstituencies.filter(
        (constituency) =>
  ...
  const handleShare = async (constituency: ConstituencyData): Promise<void> => {
    const constituencySlug = constituency.constituencyName.en
      .toLowerCase()
      .replace(/\s+/g, "-");
  ...
  const loadMore = () => {
    setVisibleConstituencies((prev) =>
      Math.min(prev + 2, filteredAndSortedConstituencies.length),
    );
  };

  // Update satisfaction No percentage
  const constituenciesToShow = filteredAndSortedConstituencies
    .slice(0, visibleConstituencies)
    .map((c) => ({
      ...c,
        ...
        <div className="w-full max-w-none mx-auto">
          <div className="text-center">
            <div className="grid grid-cols-2 gap-6 md:gap-20 items-center mb-6 sm:mb-8">
              ...
              <div className="text-center">
                <h1 className="text-3xl max-[340px]:text-2xl sm:text-4xl md:text-6xl font-bold leading-tight text-left">
                  जनता का
                </h1>
                <h1 className="text-3xl max-[340px]:text-2xl sm:text-4xl md:text-6xl font-bold leading-tight text-left">
                  चुनावी <span className="text-red-400">साथी</span>
                </h1>
              </div>
              <div className="text-center">
                <p className="text-base max-[400px]:text-xs max-[330px]:text-[8px] sm:text-lg md:text-xl font-medium text-right">
                  ...
                  {filteredAndSortedConstituencies.length > 0 ? (
                    ...
                    <div className="px-4 py-2 text-sm text-slate-500">
                      {isEnglish
                        ...
                        : "कोई निर्वाचन क्षेत्र नहीं मिला"}
                    </div>
                  ) : (
          ...
          <div className="w-full max-w-none mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gradient-to-r from-sky-50 to-blue-50 rounded-xl p-6 border border-sky-200">
              <div className="text-center mb-6">
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">
                  {isEnglish
                    ...
                    : "चयनित निर्वाचन क्षेत्र"}
                </h2>
                <p className="text-slate-600">
                  {isEnglish
                    ? "Detailed information about your selected constituency"
                    : "आपके चयनित निर्वाचन क्षेत्र की विस्तृत जानकारी"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-lg p-6 shadow-md border border-slate-200">
                  <div className="text-center mb-4">
                    ...
                    <div
                      className={`inline-block px-3 py-1 rounded-full text-sm font-medium text-white ${selectedConstituency.partyName.color}`}
                    >
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-600">
                        {isEnglish ? "Constituency:" : "निर्वाचन क्षेत्र:"}
                      </span>
                      <span className="font-medium">
                        {selectedConstituency.constituencyName.hi}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">
                        {isEnglish ? "Experience:" : "अनुभव:"}
                      </span>
                      <span className="font-medium">
                        {selectedConstituency.experience.hi}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">
                      ...
                      <span className="font-medium">
                        {selectedConstituency.education.hi}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${selectedConstituency.satisfactionYes}%`,
                        }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center mt-6">
                <button
                  onClick={() =>
                    navigateToConstituencyPage(selectedConstituency)
                  }
        ...
        <div className="w-full max-w-none mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">
              {isEnglish
                ? "Featured Constituencies"
                : "चर्चित निर्वाचन क्षेत्र"}
            </h2>
            <p className="text-slate-600">
              {isEnglish
                ? `Showing ${Math.min(12, allConstituencies.length)} of ${allConstituencies.length} constituencies`
                : `${Math.min(12, allConstituencies.length)} में से ${allConstituencies.length} निर्वाचन क्षेत्र दिखा रहे हैं`}
            </p>
          </div>
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div
                      className={`inline-block px-2 py-1 rounded-full text-xs font-medium text-white mt-1 ${constituency.partyName.color}`}
                    >
                      {constituency.partyName.nameHi}
                    </div>
                  </div>
            ...
            <button
              onClick={() => {
                ...
                window.location.href =
                  "/constituency/all-constituencies?showAll=true";
              }}
              className="bg-sky-600 text-white px-8 py-4 rounded-lg hover:bg-sky-700 transition-colors font-medium text-lg"
            >
              {isEnglish
                ? `View All ${allConstituencies.length} Constituencies`
                : `सभी ${allConstituencies.length} निर्वाचन क्षेत्र देखें`}
            </button>
            <p className="text-slate-500 mt-3 text-sm">
              {isEnglish
                ? "Get detailed information about every constituency in Bihar"
                : "बिहार के हर निर्वाचन क्षेत्र की विस्तृत जानकारी प्राप्त करें"}
            </p>
      ...
      <div className="w-full bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-200">
        <div className="w-full max-w-none mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h3 className="text-lg font-semibold text-center mb-4 text-amber-800">
            {isEnglish
              ? "🔥 Trending Topics in Bihar"
              : "🔥 बिहार में चर्चित विषय"}
          </h3>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border border-slate-100 hover:shadow-lg transition-all duration-300 hover:scale-105 card-hover">
              ...
              <div className="space-y-3">
                {recentActivities.map((activity, index) => (
                  ...
                  >
                    <div className="text-sky-600 mt-1">{activity.icon}</div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {tiers.map((tier, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-lg shadow-md p-4 sm:p-6 hover:shadow-lg transition-all duration-300 hover:scale-105 hover:-translate-y-2 card-hover stagger-item border border-slate-100"
                  >
            ...
            <section>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
                {achievements.map((achievement, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-lg shadow-md p-3 sm:p-6 text-center hover:shadow-lg transition-all duration-300 hover:scale-105 hover:-translate-y-2 card-hover stagger-item border border-slate-100"
                  >
            ...
            <section>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                {constituenciesToShow.map((constituency) => (
                  <div
                    key={constituency.id}
                    className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 hover:scale-105 hover:-translate-y-2 card-hover stagger-item border border-slate-100"
                  >
                    {/* Header with Party Color */}
                    <div
                      className={`${constituency.partyName.color} h-2`}
                    ></div>
                    ...
                    <div className="p-4 sm:p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 mb-4 text-xs sm:text-sm">
                        <div>
                          ...
                          {isEnglish
                            ? constituency.experience.en
                            : constituency.experience.hi}
                        </div>
                        <div>
                          <span className="font-medium">
                            {content.education}:
                      ...
                      <div className="mb-4">
                        ...
                        <div className="w-full bg-slate-200 rounded-full h-2">
                          <div
                            className="bg-emerald-500 h-2 rounded-l-full transition-all duration-500"
                            style={{
                              width: `${constituency.satisfactionYes}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                ...
                <div className="text-center py-12">
                  <p className="text-slate-500">
                    {isEnglish
                      ...
                      : "आपकी खोज से मेल खाने वाला कोई निर्वाचन क्षेत्र नहीं मिला।"}
                  </p>
                </div>
              )}
            </section>
